...
export const PushDropdown = ({
  ...
  useEffect(() => {
    ...
    } else if (searchable && searchText.toLowerCase() === value.toLowerCase()) {
      setOptions(data);
  ...
  return (
    <div className={styles.input}>
      ...
      <div className={styles.inputWrapper} onClick={toggle}>
        ...
        {loading ? (
          <Spinner size={20} color="#667085" />
        ) : (
          <>
            {isOpen ? (
              <MdOutlineKeyboardArrowUp className={styles.icon} size={20} onClick={toggle} />
            ) : (
        ...
        {options.map((current) => {
          ...
          return (
            <div
              id={randomId}
              key={randomId}
              className={classNames(styles.row, current === value ? styles.selected : '')}
              onClick={() => {
                onChange(current);
                toggle();
            ...
            >
              ...
              {current === value && <AiOutlineCheck color="#0D3DDC" />}
            </div>
          );
        })}
      </div>
